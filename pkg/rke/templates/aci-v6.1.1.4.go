...
{{- end}}
        ...
        "enable-apic-request-retry-delay": {{.EnableApicRequestRetryDelay}},
        "apic-request-retry-delay": {{.ApicRequestRetryDelay}}
    }
  host-agent-config: |-
    {
        "app-profile": "aci-containers-{{.SystemIdentifier}}",
...
{{- end}}
{{- if ne .EpgResolvePrioritize "false" }}
        "epg-resolve-prioritize": {{.EpgResolvePrioritize}},
{{- end}}
{{- if ne .EnableOpflexAgentReconnect "false"}}
        "enable-opflex-agent-reconnect": {{.EnableOpflexAgentReconnect}},
{{- end}}
...
{{- end}}
          ...
          "force-ep-undeclares": { "enabled": "{{ .ForceEpUndeclares }}" }
{{- if ne .EnableHppDirect "false"}}
          ,"enable-local-netpol": {{.EnableHppDirect}}
{{- end}}
{{- if ne .EpgResolvePrioritize "false" }}
          ,"epg-resolve-prioritize": { "enabled" : "{{.EpgResolvePrioritize}}" }
{{- end}}
        },
        "ovs": {
