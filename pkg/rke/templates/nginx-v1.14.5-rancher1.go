...
/*
Upstream Changelog:
- update app.kubernetes.io/version label value from 1.14.4 to 1.14.5
- Updated admission webhook jobs to use kube-webhook-certgen v1.6.9
...
- other Rancher specific changes, are same as NginxIngressTemplateV1144Rancher1
*/
const NginxIngressTemplateV1145Rancher1 = `
# Based on https://github.com/kubernetes/ingress-nginx/blob/controller-v1.14.5/deploy/static/provider/cloud/deploy.yaml
apiVersion: v1
kind: Namespace
metadata:
  labels:
      ...
      containers:
        - name: controller
          ...
          args:
            ...
            # Rancher specific: Intentionally added and marked as true
            - --enable-metrics=true
          {{- range $k, $v := .ExtraArgs }}
            - --{{ $k }}{{if ne $v "" }}={{ $v }}{{end}}
          {{- end }}
