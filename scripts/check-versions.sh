...
# This Bash script checks whether any released RKE2 or K3s versions
...
if [[ -z "$1" ]]; then
    echo "Check if any released RKE2/K3s version from the target branch is missing in the local data.json file"
    echo "Usage:"
    echo "  $0 <branch-name>"
    exit 1
fi
